...
/**
 * @author Milan 18 мая 2014 г. - 1:12:04
 * 
 */
@Repository
public class EmergencyDaoImpl implements EmergencyDao {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(EmergencyDaoImpl.class);
	...
	public Emergency findEmergencyById(int emergencyId)
		...
		List<Emergency> results = query.getResultList();
		Emergency em = null;
		if (results != null && results.size() > 0) {
			em = results.get(0);
			for (Action action : em.getActions()) {
	...
	// String query =
	// "SELECT * FROM events left JOIN accidents ON events.event_id=accidents.event_id left join dangers on events.event_id=dangers.event_id";
	...
	// Accident emergency = new Accident( rs.getInt("event_id"), new
	// DateTime(rs.getDate("created_date_and_time")), rs.getString("place"),
	// rs.getString("text_description"), rs.getInt("owner_id"),
	// rs.getString("freshness"), rs.getString("certification"),
